...
<script>
    document.addEventListener('DOMContentLoaded', function () {
		...
		// Function to fetch existing access users and populate the textarea
		function fetchAccessUsers(filename) {
			// Send an AJAX request to access.php to fetch existing access users
			...
			xhr.onreadystatechange = function () {
				if (xhr.readyState == 4 && xhr.status == 200) {
					...
					if (response.success) {
						// Update the textarea with existing access users
						document.getElementById('accessTextarea').value = response.accessUsers;
					} else {
						// Handle error if needed
						console.error('Error fetching access users');
					}
				}
			};
			xhr.send('filename=' + encodeURIComponent(filename)); // Include the filename parameter
		}
